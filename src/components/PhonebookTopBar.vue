  ...
  <div class="top-bar" aria-label="PhonebookTopBar">
    ...
    <div class="button-container">
      <button @click="handleSortChange" class="sort-button" aria-label="sort">
        <FontAwesomeIcon :icon="sort === 'asc' ? faArrowDownZA : faArrowDownAZ" size="lg" />
      </button>
    </div>
    ...
    <div class="search-container" aria-label="search">
      ...
      <input v-model="keyword" @input="handleSearchChange" class="search-input" type="text" placeholder="Search"
    ...
    <div class="form-container">
      <button @click="goToFormScreen" class="add-button">
        <FontAwesomeIcon :icon="faUserPlus" size="lg" color="black" />
      </button>
    </div>
  </div>
</template>
...
export default defineComponent({
  ...
  setup() {
    ...
    const { refreshPhonebookData, clearPhonebook } =
      phonebookStore;
    ...
    const handleSearchChange = () => {
      refreshPhonebookData({ keyword: phonebookStore.keyword, sort: phonebookStore.sort, page: 1 });
    };
